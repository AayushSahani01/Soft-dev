    ...
    <aside className="w-full col-span-3 pt-4 hidden md:block m-1.5 space-y-4">
      ...
      <div className="bg-slate-50 shadow-md rounded-sm px-4 py-6">
        ...
        <div className="flex gap-2">
          {["red", "blue", "green", "yellow", "black", "white"].map((clr) => (
            <button
              key={clr}
              onClick={() => setColor(clr)}
              className={`w-7 h-7 mr-2 bg-${clr}-500 cursor-pointer transition-all duration-300 ease-in-out rounded-full border border-transparent hover:border-slate-600 focus:outline-none
            }
            )} ${color === clr ? "ring-2 ring-offset-4 ring-sky-500" : ""}`}
            />
          ))}
        </div>
      </div>
